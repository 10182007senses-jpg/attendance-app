...
## 各ファイルの変更内容要約
- db.py: User.break_rule 列追加、既存DB向け ALTER TABLE users ADD COLUMN break_rule、null/空値を standard で補完。
- main.py: 休憩ルール定数とバリデーション追加、/admin/users GET/POST/PATCH を拡張、calc_day_from_logs(..., break_rule=...) に変更し、日次・月次・管理集計へユーザー別ルールを伝播。
- static/admin.html: ユーザー管理に「休憩ルール」列と新規作成欄を追加し、一覧変更イベントで break_rule 更新を追加。

## 手動確認手順
1. ユーザー追加時の休憩ルール設定
   - 管理者画面のユーザー管理で新規ユーザー名・PIN・権限・休憩ルールを入力して追加。
   - /admin/users の一覧に break_rule が保存され、選択した値が表示されることを確認。
2. 既存ユーザーの休憩ルール変更
   - 既存ユーザー行の「休憩ルール」を 通常 / 常に1時間 に切り替える。
   - 変更後に一覧再読込しても設定が保持されることを確認。
3. standard と always_1h の両方で勤務時間計算が変わることの確認
   - 5時間30分勤務データで standard は休憩 0、always_1h は実働 4時間30分になることを確認。
   - 6時間30分勤務データで両方とも1時間控除になることを確認。
   - 30分勤務データで always_1h は実働 0時間0分になり、マイナスにならないことを確認。
   - 月次詳細、月次サマリー、CSV/XLSX 出力でも同じ実働になることを確認。
4. ユーザー管理一覧表示の確認
   - 列順が ID / ユーザー / 権限 / 休憩ルール / 状態 / 操作 になっていることを確認。
   - ユーザー0件時のメッセージ行が崩れないことを確認。
5. 既存DBに対する列追加の確認ポイント
   - アプリ起動後に users テーブルへ break_rule 列が追加されること。
   - 既存ユーザーの break_rule が standard で埋まること。
   - 既存の workdays.employee_note 後付け追加処理が壊れていないこと。
    ...
    <div class="app">
      ...
      <div class="admin-grid">
        ...
        <div class="panel">
          ...
          <div class="hint">ユーザー追加 / 無効化 / 権限変更 / 休憩ルール変更 / PIN再設定</div>
          ...
          <div id="user-panel-content">
            <div class="field-row">
              ...
              <div>
                <label class="login-label">休憩ルール</label>
                <select id="new-user-break-rule">
                  <option value="standard">通常</option>
                  <option value="always_1h">常に1時間</option>
                </select>
              </div>
    ...
    <script>
      ...
      function renderUsers(users) {
        ...
        users.forEach(u => {
          ...
          const breakRule = u.break_rule || "standard";
          ...
          tr.innerHTML = `
            ...
            <td>
              <select data-action="break_rule" data-name="${escapeHtml(u.name)}">
                <option value="standard" ${breakRule === "standard" ? "selected" : ""}>通常</option>
                <option value="always_1h" ${breakRule === "always_1h" ? "selected" : ""}>常に1時間</option>
              </select>
            </td>
            <td>
      ...
      async function createUser() {
        ...
        const break_rule = document.getElementById("new-user-break-rule").value.trim();
        ...
        const res = await fetchWithAuth("/admin/users", {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({name, pin, role, break_rule}),
        });
        if (!res) return;
      ...
      document.getElementById("users-table").addEventListener("change", async (event) => {
        ...
        if (action === "break_rule") {
          setUserStatus("休憩ルールを更新中...");
          await updateUser({ name, break_rule: target.value });
          setUserStatus("更新しました", "ok");
          return;
        }
      })
